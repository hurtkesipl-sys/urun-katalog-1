...
export default function Navbar() {
  ...
  return (
    <nav className={`border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 sticky top-0 z-50 transition-transform duration-300 ${isVisible ? 'translate-y-0' : '-translate-y-full'}`}>
      <div className="container flex flex-col items-center py-1.5 md:py-2 relative">
        ...
        <div className="w-full flex justify-between items-center mb-1 md:mb-2">
          ...
          <div className="w-28 hidden md:flex items-center gap-3">
          ...
          <div className="flex flex-col items-center ml-2 md:ml-0 flex-1 md:flex-none overflow-hidden">
            <Link href="/" className="flex flex-col items-center mb-0.5 md:mb-1">
              <span className="font-serif text-xl md:text-2xl font-bold tracking-tight whitespace-nowrap" translate="no">Moda İtalya</span>
              {/* İtalyan Bayrağı Çizgisi */}
              <div className="flex w-24 md:w-full h-1 md:h-1.5 rounded-full overflow-hidden mt-0.5 mb-0.5">
                <div className="w-1/3 bg-green-600"></div>
                <div className="w-1/3 bg-white border-y border-gray-200"></div>
                <div className="w-1/3 bg-red-600"></div>
              </div>
              <span className="text-muted-foreground text-[10px] md:text-xs font-medium truncate">Toptan</span>
            </Link>
          </div>
          ...
          <div className="flex flex-col items-end gap-1 w-auto md:w-24 justify-end shrink-0">
            <div className="flex items-center gap-2 md:gap-4">
              {isTranslationEnabled && (
                <div className="flex items-center gap-1.5 md:gap-1.5 mr-2 md:mr-3 relative z-50">
                  <button onClick={() => changeLanguage('tr')} className="w-6 h-6 md:w-7 md:h-7 hover:scale-110 transition-transform cursor-pointer flex items-center justify-center" title="Türkçe">
                  ...
                  <button onClick={() => changeLanguage('en')} className="w-6 h-6 md:w-7 md:h-7 hover:scale-110 transition-transform cursor-pointer flex items-center justify-center" title="English">
                  ...
                  <button onClick={() => changeLanguage('ar')} className="w-6 h-6 md:w-7 md:h-7 hover:scale-110 transition-transform cursor-pointer flex items-center justify-center" title="العربية">
                  ...
                  <button onClick={() => changeLanguage('ru')} className="w-6 h-6 md:w-7 md:h-7 hover:scale-110 transition-transform cursor-pointer flex items-center justify-center" title="Русский">
              ...
              <div className="flex flex-col items-end gap-0.5 md:gap-1">
                ...
                <div className="hidden md:flex gap-2 text-[11px] font-medium text-muted-foreground items-center">
        ...
        <div className="w-full hidden md:flex flex-col items-center gap-1">
